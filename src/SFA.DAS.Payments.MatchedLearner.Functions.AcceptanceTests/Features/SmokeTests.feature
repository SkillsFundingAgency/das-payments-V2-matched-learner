...
Scenario: DataLockEvents-Tests-Function pulls the DataLockEvents from Payments DB and saves to matched learner DB
...
Scenario: DataLockEvents-Tests-Existing DataLockEvents for current collection data is deleted before saving new Data to matched learner DB
...
Scenario: DataLockEvents-Tests-Existing DataLockEvents for previous collection then data is deleted before saving new Data to matched learner DB
...
Scenario: DataLockEvents-Tests-Existing DataLockEvents for previous Academic year latest collection then only new data is saved to matched learner DB
	Given A Submission Job Succeeded for CollectionPeriod 1 and AcademicYear 2122
...
Scenario: DataLockEvents-Tests-Function pulls the DataLockEvents from Payments DB and saves to matched learner DB for Same Collection Period
	Given A Submission Job Succeeded for CollectionPeriod 1 and AcademicYear 2122
